<template>
  ...
  <el-dialog title="添加目录" :visible.sync="visible_" width="30%" :show-close="false">
    ...
    <!-- 底部区域 -->
    <span slot="footer">
      <el-button round @click="cancel('formRef')">取消</el-button>
      <el-button round type="primary" @click="add()">确定</el-button>
    </span>
  </el-dialog>
</template>
      ...
      this.$refs.formRef.validate(async valid => {
        if (valid) {
          ...
          if (result.status !== 200) {
            ...
            // 自定义失败事件
            return this.$emit('error', result.message, 'add')
          }
          // 自定义成功事件
          this.$refs['formRef'].resetFields()
          this.$emit('success', result.message, 'add')
        } else {
          // 自定义警告事件
          this.$emit('warn', '请正确填写表单信息！', 'add')
        }
      })
    },
    // 取消添加
