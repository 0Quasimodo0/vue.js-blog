    ...
    <el-card shadow="never">
      ...
      <el-form :model="form.article" :rules="form.rules" ref="infoFormRef">
        ...
        <el-form-item label="* 封面" prop="cover">
          <el-image v-if="Switch.previewCover" style="width: 240px; height: 135px;" :src=" temp.url_prefix + temp.url_content" fit="contain"></el-image>
          <el-input placeholder="请输入内容" v-model="temp.url_content" clearable>
            <template slot="prepend">Http://</template>
            <el-button slot="append" icon="el-icon-view" @click="Switch.previewCover = !Switch.previewCover">预览</el-button>
          </el-input>
        </el-form-item>
      ...
      <div class="button-area">
        ...
        <div class="button-right">
          <el-button type="success" round icon="el-icon-check" @click="submit">提交</el-button>
          <el-button type="primary" round icon="el-icon-delete" @click="reset('infoFormRef')">清空</el-button>
        </div>
      </div>
    </el-card>
  </div>
...
export default {
    ...
    return {
      ...
      form: {
        ...
        rules: {
          ...
          // 验证是否输入标签
          tags: [
            { type: 'array', required: false, message: '请至少输入一个标签', trigger: 'change' }
          ]
        }
      },
      // 选择器
  ...
  methods: {
    ...
    submit () {
      this.$refs.infoFormRef.validate(async valid => {
        if (this.form.article.contentMd !== '' && valid) {
          ...
          this.form.cover = this.temp.url_prefix + this.temp.url_content
          // console.log(this.form.article)
          const { data: result } = await this.$http.post('/admin/article', this.form.article)
          if (result.status !== 200) {
    ...
    reset (refName) {
      ...
      this.form.article.contentMd = ''
      this.form.article.contentHtml = ''
    }
  }
}
</script>
